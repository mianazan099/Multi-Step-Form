...
export default function Summary({
  ...
  const selectedPlanPrice = monthly
    ? data.plans[plan].price.monthly
    : data.plans[plan].price.yearly;
  ...
  return (
    <>
      ...
      <div className="rounded-md bg-purplishBlue bg-opacity-5 p-4 pt-5 text-sm lg:p-6">
        ...
        >
          <p className="font-medium text-marineBlue text-base pb-1">
            {selectedPlan.name} {monthly ? "(Monthly)" : "(Yearly)"}
          </p>
          <p className="font-bold text-marineBlue text-base">
            ${selectedPlanPrice}/{monthly ? "mo" : "yr"}
          </p>
        </div>
        <div
          className={`space-y-3 mt-4 lg:space-y-4 lg:mt-5 ${
            ...
            return (
              ...
              >
                ...
                <p className="text-marineBlue">
                  +${monthly ? addonData.price.monthly : addonData.price.yearly}
                  /{monthly ? "mo" : "yr"}
                </p>
              </div>
            );
      ...
      <div className="mt-6 flex items-center justify-between px-4 lg:px-6">
        ...
        <p className="text-purplishBlue font-bold text-xl">
          +${selectedPlanPrice + addonsPriceSum}/{monthly ? "mo" : "yr"}
        </p>
      </div>
    </>
  );
