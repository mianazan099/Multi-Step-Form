...
function Step({ num, name, currentStep, goto }: StepProps) {
    return (
        <button className="lg:flex lg:items-center lg:gap-4" onClick={() => { goto(num) }}>
            <span className={`${num === currentStep ? "border-lightBlue bg-lightBlue" : "border-white text-white"} inline-grid h-8 w-8 place-content-center rounded-full border`}>
                {num + 1}
            </span>
